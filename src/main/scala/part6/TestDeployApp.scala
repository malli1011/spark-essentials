...
object TestDeployApp {
   ...
   *   - (important) then from the generated folder path, delete the folder path after src/ so that the folder path ends in src/
   *
   *
   * Build the JAR: Build -> Build Artifacts... -> select the jar -> build
   * Copy the JAR spark-essentials.jar and movies.json to spark-cluster/apps and /opt/spark-data respectively
  ...
  /*
  get docker id from the terminal after starting the spark containers.
  * /spark/bin/spark-submit --class part6.TestDeployApp --master spark://921d7b34612f:7077 --deploy-mode client  --verbose --supervise /opt/spark-apps/spark-essentials.jar /opt/spark-data/movies.json /opt/spark-data/goodMovies
  * */
}